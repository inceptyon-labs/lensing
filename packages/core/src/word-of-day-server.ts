...
function parseWotdFeed(xml: string): WordOfDayData | null {
  ...
  for (const line of lines) {
    if (!capturing) {
      ...
      if (posMatch && line.includes(posMatch[0])) {
        ...
        if (afterPos) definition = afterPos;
        capturing = true;
        continue;
      }
...
export function createWordOfDayServer(options: WordOfDayServerOptions): WordOfDayServerInstance {
  const { dataBus, fetchFn } = options;

  const effectiveFetch = (fetchFn ?? fetch) as unknown as WotdFetchFn;

  let lastData: WordOfDayData | null = null;
  let closed = false;
  const updateListeners: Array<(data: WordOfDayData) => void> = [];
  const errorListeners: Array<(error: string) => void> = [];

  function notifyUpdate(data: WordOfDayData): void {
  ...
  async function refresh(): Promise<void> {
    ...
    lastData = parsed;
    (dataBus as DataBusInstance).publish(DATA_BUS_CHANNEL, PLUGIN_ID, parsed);
    notifyUpdate(parsed);
  }
