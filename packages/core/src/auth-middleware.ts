 ...
 */
export function isProtectedRoute(path: string, method: string): boolean {
  if (method === 'OPTIONS') return false;
  if (path === '/health') return false;

  // Read-only marketplace browsing is public
  if (method === 'GET' && path === '/marketplace') return false;
  if (method === 'GET' && path === '/marketplace/categories') return false;
  if (method === 'GET' && path === '/marketplace/updates') return false;
