...
function extractHref(xml, wrapperTag) {
    const wrapperRe = new RegExp(`<[a-zA-Z]*:?${wrapperTag}[^>]*>([\\s\\S]*?)<\\/[a-zA-Z]*:?${wrapperTag}>`, 'i');
...
export async function discoverCalendars(serverUrl, authHeader, fetchFn, timeoutMs) {
    const propfind = async (url, body) => {
        ...
        try {
            ...
            if (!res.ok && res.status !== 207)
                throw new Error(`PROPFIND ${url} returned ${res.status}`);
    ...
    const propfindList = async (url, body) => {
        const controller = new AbortController();
        const tid = setTimeout(() => controller.abort(), timeoutMs);
        try {
            const res = await fetchFn(url, {
    ...
    if (!principalHref)
        throw new Error('CalDAV discovery: could not find current-user-principal');
    ...
    if (!homeHref)
        throw new Error('CalDAV discovery: could not find calendar-home-set');
    ...
    if (calendars.length === 0)
        throw new Error('CalDAV discovery: no calendars found');
    return calendars;
}
// ── Factory ───────────────────────────────────────────────────────────────────
/**
...
export function createCalendarServer(options) {
    ...
    async function fetchOneCalendar(target, now, end, attempt = 0) {
    ...
    async function doFetch() {
        ...
        console.log(`[CalDAV] Fetching ${resolvedTargets.length} calendars:`, resolvedTargets.map((t) => t.name));
        const results = await Promise.all(resolvedTargets.map((t) => fetchOneCalendar(t, now, end)));
        const events = results.flat();
        console.log(`[CalDAV] Total events found: ${events.length}`, events.map((e) => `${e.calendar}: ${e.title}`));
        lastEvents = events;
