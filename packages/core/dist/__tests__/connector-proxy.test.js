...
import { testConnector } from '../connector-proxy';
/** Helper: create a mock fetch that returns JSON */
function mockJsonFetch(data, status = 200) {
...
function mockTextFetch(text, status = 200) {
    return vi.fn().mockResolvedValue({
        ok: status >= 200 && status < 300,
        status,
        statusText: status === 200 ? 'OK' : 'Error',
...
describe('testConnector', () => {
    describe('validation', () => {
        it('should reject missing type', async () => {
            const result = await testConnector({ type: '', url: 'https://example.com' });
    ...
    describe('SSRF protection', () => {
        ...
        it('should allow private IPs when allowPrivate is true', async () => {
            ...
            const result = await testConnector({ type: 'json_api', url: 'http://192.168.1.100/api' }, { fetchFn, allowPrivate: true });
    ...
    describe('JSON API connector', () => {
        it('should fetch and return sample data', async () => {
            ...
            const result = await testConnector({ type: 'json_api', url: 'https://api.example.com/weather' }, { fetchFn });
            expect(result.success).toBe(true);
        ...
        it('should extract field paths from array items', async () => {
            ...
            const fetchFn = mockJsonFetch(data);
            const result = await testConnector({ type: 'json_api', url: 'https://api.example.com/list' }, { fetchFn });
    ...
    describe('RSS connector', () => {
        it('should fetch and return parsed RSS items', async () => {
            ...
            const result = await testConnector({ type: 'rss_feed', url: 'https://example.com/feed.xml' }, { fetchFn });
            expect(result.success).toBe(true);
    ...
    describe('error handling', () => {
        it('should return error on HTTP failure', async () => {
            ...
            const result = await testConnector({ type: 'json_api', url: 'https://api.example.com/data' }, { fetchFn });
            expect(result.success).toBe(false);
        ...
        it('should return error on timeout', async () => {
            ...
            const result = await testConnector({ type: 'json_api', url: 'https://api.example.com/slow' }, { fetchFn, timeoutMs: 50 });
            expect(result.success).toBe(false);
            expect(result.error).toMatch(/timeout/i);
        });
    });
});
//# sourceMappingURL=connector-proxy.test.js.map