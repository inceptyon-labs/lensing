...
describe('createPublisherPr', () => {
    ...
    it('should commit ZIP file to plugins/<id>/<id>-<version>.zip', async () => {
        ...
        const zipCall = mockFetch.mock.calls.find((call) => call[0]?.includes('plugins/my-plugin/my-plugin-1.5.0.zip'));
    ...
    it('should commit thumbnail to plugins/<id>/thumbnail.png', async () => {
        ...
        const thumbCall = mockFetch.mock.calls.find((call) => call[0]?.includes('plugins/my-plugin/thumbnail.png'));
    ...
    it('should update index.json with plugin metadata', async () => {
        ...
        const indexOptions = mockFetch.mock.calls.find((call) => call[0]?.includes('contents/index.json'))?.[1];
    ...
    it('should create PR with descriptive title and body', async () => {
        ...
        const prOptions = mockFetch.mock.calls.find((call) => call[0]?.includes('/pulls') && !call[0]?.includes('/pulls/'))?.[1];
    ...
    it('should validate required config fields', async () => {
        await expect(createPublisherPr({ ...defaultConfig, githubToken: '' })).rejects.toThrow(/required|missing|token/i);
    });
    it('should reject owner with path traversal characters', async () => {
        await expect(createPublisherPr({
            ...defaultConfig,
