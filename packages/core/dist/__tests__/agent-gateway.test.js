...
function createMockDataBus() {
    ...
    return {
        publish: vi.fn(),
        subscribe: vi.fn(() => () => { }),
        getLatest: vi.fn((channel) => channels.get(channel)),
...
function createMockResult(response = 'Test response') {
    return {
        response,
        tool_calls_made: 0,
        audit_entries: [],
...
describe('Agent Gateway', () => {
    ...
    function createGateway(overrides = {}) {
        gateway = createAgentGateway({
            ...
            onResponse,
            onStatusChange,
            createWebSocket: getMockWebSocketFactory(),
            ...overrides,
        });
        return gateway;
    }
    ...
    describe('Data Bus Snapshot Forwarding', () => {
        it('should respond to data_snapshot_request with channel data', () => {
            ...
            dataBus.getLatest.mockReturnValue(mockSnapshot);
            createGateway();
            gateway.connect();
            const ws = getLatestMockWs();
            ws.simulateOpen();
...
//# sourceMappingURL=agent-gateway.test.js.map