...
describe('WeatherServer', () => {
    describe('factory and config', () => {
        ...
        it('should throw if apiKey is missing for OpenWeatherMap', () => {
            expect(() => createWeatherServer(validOptions({ provider: 'openweathermap', apiKey: '' }))).toThrow(/apiKey/i);
        });
        ...
        it('should throw if location is missing', () => {
            expect(() => createWeatherServer(validOptions({ location: undefined }))).toThrow(/location/i);
        });
    ...
    describe('fetch and transform', () => {
        function owmForecastDay(dt, max, min, desc, icon) {
        ...
        it('should include lat/lon/apiKey in URL', () => {
            ...
            const server = createWeatherServer(validOptions({ fetchFn, location: { lat: 51.5074, lon: -0.1278 } }));
            server.refresh();
        ...
        it('OpenWeatherMap vendor limitation: appid must stay in URL (no header auth supported)', () => {
            ...
            const calledInit = fetchFn.mock.calls[0][1];
    ...
    describe('cache integration', () => {
        ...
        it('should re-fetch after cache expires', async () => {
            ...
            const server = createWeatherServer(validOptions({ fetchFn, maxStale_ms: 0 }) // stale immediately
            );
            await server.refresh();
    ...
    describe('dataBus publishing', () => {
        it('should publish weather data to dataBus on successful refresh', async () => {
            ...
            expect(publish).toHaveBeenCalledWith('weather.current', 'weather-server', expect.objectContaining({
                current: expect.any(Object),
                forecast: expect.any(Array),
                lastUpdated: expect.any(Number),
            }));
    ...
    describe('Open-Meteo provider', () => {
        it('should build Open-Meteo URL with correct params', () => {
            ...
            const server = createWeatherServer(openMeteoOptions({ fetchFn, location: { lat: 51.5074, lon: -0.1278 } }));
            server.refresh();
    ...
    describe('geocoding via locationQuery', () => {
        ...
        it('should cache geocoded coordinates (no re-geocode on second refresh)', async () => {
            ...
            const geocodeCalls = fetchFn.mock.calls.filter((c) => c[0].includes('geocoding-api.open-meteo.com'));
...
//# sourceMappingURL=weather-server.test.js.map