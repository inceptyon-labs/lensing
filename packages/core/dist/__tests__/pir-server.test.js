...
// ── Mock GPIO helpers ────────────────────────────────────────────────────────
function createMockGpioWatcher() {
...
function createMockGpioFactory(watcher) {
    const mockWatcher = watcher ?? createMockGpioWatcher();
    const factory = vi.fn(() => mockWatcher);
...
describe('PIR Server', () => {
    ...
    it('should publish to presence.pir data bus channel on motion', () => {
        ...
        expect(presenceCall[2].detected).toBe(true);
        server.close();
    });
    // ── Idle timeout ────────────────────────────────────────────────────────────
    ...
    it('should publish idle state to data bus', () => {
        ...
        expect(lastCall[2].detected).toBe(false);
        server.close();
    });
    ...
    it('should pass configured GPIO pin to factory', () => {
        const factoryFn = vi.fn(() => createMockGpioWatcher());
    ...
    it('should close watcher if watch() throws after successful creation', () => {
        ...
        const factory = vi.fn(() => throwingWatchWatcher);
        const server = createPIRServer({ dataBus, gpioFactory: factory });
...
//# sourceMappingURL=pir-server.test.js.map