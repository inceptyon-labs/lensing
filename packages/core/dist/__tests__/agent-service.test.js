...
function createMockSceneManager() {
    return {
        ...
        switchTo: vi.fn((name) => ({ name, layout: [], active_plugins: [] })),
        addScene: vi.fn(),
        updateScene: vi.fn(),
        removeScene: vi.fn(),
        onSceneChange: vi.fn(() => () => { }),
...
describe('AgentService', () => {
    ...
    describe('built-in tool execution', () => {
        it('should execute query_data_bus tool', async () => {
            dataBus.getLatest.mockReturnValue({
                channel: 'weather',
                data: { temp: 72 },
        ...
        it('should execute list_channels tool', async () => {
            dataBus.getChannels.mockReturnValue([
                'weather',
                'calendar',
                'news',
            ]);
    ...
    describe('audit log', () => {
        ...
        it('should record errors in audit log', async () => {
            sceneManager.switchTo.mockImplementation(() => {
    ...
    describe('agent loop (executeTask)', () => {
        ...
        it('should handle single-turn tool calling', async () => {
            dataBus.getChannels.mockReturnValue(['weather']);
            llm = createMockLlm([
                {
                    content: '',
        ...
        it('should limit tool call iterations to prevent infinite loops', async () => {
            ...
            llm = createMockLlm(Array(20).fill({
                content: '',
                stop_reason: 'tool_use',
    ...
    describe('morning brief', () => {
        it('should gather data from data bus channels and generate brief via LLM', async () => {
            ...
            dataBus.getLatest.mockImplementation((ch) => ({
                channel: ch,
        ...
        it('should handle channels with no data', async () => {
            dataBus.getChannels.mockReturnValue(['weather', 'calendar']);
            dataBus.getLatest.mockReturnValue(undefined);
            llm = createMockLlm([
                {
    ...
    describe('condition evaluation', () => {
        ...
        it('should emit notifications for alerts with action=notify', () => {
            ...
            expect(notificationQueue.emit).toHaveBeenCalledWith(expect.objectContaining({
                source: 'agent',
                title: 'Alert!',
            }));
        });
    ...
    describe('edge cases', () => {
        ...
        it('should isolate notification emission exceptions', () => {
            notificationQueue.emit.mockImplementation(() => {
...
//# sourceMappingURL=agent-service.test.js.map