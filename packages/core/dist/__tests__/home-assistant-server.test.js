...
function makeHaState(entity_id, state, attributes = {}) {
    return {
        entity_id,
        state,
...
function createErrorFetch(status) {
    return vi.fn().mockResolvedValue({
        ok: false,
        status,
...
function createNetworkErrorFetch() {
...
describe('Home Assistant Server', () => {
    ...
    it('should split entities into devices and sensors', async () => {
        ...
        expect(data.devices.map((e) => e.entity_id)).toEqual(expect.arrayContaining(['light.living_room', 'switch.fan', 'lock.front_door']));
        expect(data.sensors.map((e) => e.entity_id)).toEqual(expect.arrayContaining(['sensor.temperature', 'binary_sensor.motion']));
...
//# sourceMappingURL=home-assistant-server.test.js.map