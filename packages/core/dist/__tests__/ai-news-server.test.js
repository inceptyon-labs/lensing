...
function makeSummarizeFn(summaries) {
    return vi
        .fn()
...
describe('AI News Server', () => {
    ...
    it('should throw if feedUrls is empty', () => {
        expect(() => createAiNewsServer(makeOptions({ feedUrls: [] }))).toThrow('feedUrls is required and must not be empty');
    });
    it('should throw if summarize is not provided', () => {
        expect(() => createAiNewsServer(makeOptions({ summarize: undefined }))).toThrow('summarize function is required');
    });
    it('should throw if maxItems is invalid', () => {
        expect(() => createAiNewsServer(makeOptions({ maxItems: 0 }))).toThrow('maxItems must be a positive number');
        expect(() => createAiNewsServer(makeOptions({ maxItems: NaN }))).toThrow('maxItems must be a positive number');
    });
    ...
    it('should fetch RSS, summarize, and publish data on refresh', async () => {
        ...
        expect(opts.dataBus.publish).toHaveBeenCalledWith('ai-news.summaries', 'ai-news-server', expect.objectContaining({ summaries: expect.any(Array) }));
    ...
    it('should aggregate articles from multiple feeds', async () => {
        ...
        expect(summarize).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ title: 'Headline One' })]));
        const data = server.getData();
...
//# sourceMappingURL=ai-news-server.test.js.map