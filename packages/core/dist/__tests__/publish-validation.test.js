...
describe('validatePublish', () => {
    ...
    it('should fail when plugin ID is not unique', () => {
        ...
        expect(result.errors).toContainEqual(expect.objectContaining({ field: 'id', code: 'DUPLICATE_ID' }));
    });
    it('should fail when plugin ID is empty', () => {
        const result = validatePublish({ ...validInput, id: '' }, []);
        expect(result.valid).toBe(false);
        expect(result.errors).toContainEqual(expect.objectContaining({ field: 'id', code: 'REQUIRED' }));
    });
    ...
    it('should fail when plugin version is empty', () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.errors).toContainEqual(expect.objectContaining({ field: 'version', code: 'REQUIRED' }));
    });
    it('should fail when connector URL is empty', () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.errors).toContainEqual(expect.objectContaining({ field: 'connector.url', code: 'REQUIRED' }));
    });
    it('should fail when connector type is empty', () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.errors).toContainEqual(expect.objectContaining({ field: 'connector.type', code: 'REQUIRED' }));
    });
    ...
    it('should fail when template HTML is empty', () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.errors).toContainEqual(expect.objectContaining({ field: 'html', code: 'REQUIRED' }));
    });
    ...
    it('should fail when package size exceeds 10MB', () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.errors).toContainEqual(expect.objectContaining({ field: 'size', code: 'TOO_LARGE' }));
    });
    it('should fail when template has unresolved placeholders', () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.errors).toContainEqual(expect.objectContaining({ field: 'html', code: 'UNRESOLVED_PLACEHOLDERS' }));
    });
...
//# sourceMappingURL=publish-validation.test.js.map