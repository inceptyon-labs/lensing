...
describe('HostService (host-service.ts)', () => {
    ...
    it('should boot successfully and resolve ready promise', async () => {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Boot timeout')), 5000));
    ...
    it('should persist plugin enabled state to database', async () => {
        ...
        const state = hostService.db.getPluginState('test-plugin');
    ...
    it('should boot with only system modules when no layout exists', async () => {
        ...
        expect(hostService.modules.every((m) => SYSTEM_MODULE_IDS.includes(m.id))).toBe(true);
    });
    ...
    it('should forward data bus messages to WebSocket clients as plugin_data', async () => {
        ...
        const payload = pluginMsg.payload;
    ...
    it('should persist module zone via PUT /plugins/weather/zone', async () => {
        ...
        const state = hostService.db.getPluginState('weather');
...
//# sourceMappingURL=host-service.test.js.map