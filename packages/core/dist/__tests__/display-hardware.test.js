...
function makeExec(responses) {
    return (cmd, args) => {
        ...
        for (const [pattern, resp] of Object.entries(responses)) {
            if (key.startsWith(pattern) || key.includes(pattern.split(' ')[0]))
                return resp;
        }
...
describe('createDisplayHardware', () => {
    describe('capability detection', () => {
        ...
        it('logs detected capabilities', () => {
            ...
            expect(logger.info).toHaveBeenCalledWith('Display hardware capabilities detected', expect.objectContaining({
    ...
    describe('contrast via ddcutil', () => {
        function makeContrastHw() {
            const execFn = vi.fn().mockImplementation((cmd, args) => {
                ...
                if (key.startsWith('ddcutil setvcp 12'))
                    return { stdout: '', stderr: '', status: 0 };
                if (key === 'which xrandr')
                    return { stdout: '', stderr: '', status: 1 };
                return { stdout: '', stderr: 'not found', status: 1 };
    ...
    describe('rotation via xrandr', () => {
        function makeRotationHw(currentRotation = 'normal') {
            ...
            const execFn = vi.fn().mockImplementation((cmd, args) => {
                ...
                if (key.startsWith('xrandr --output'))
                    return { stdout: '', stderr: '', status: 0 };
                if (key === 'which ddcutil')
                    return { stdout: '', stderr: '', status: 1 };
                return { stdout: '', stderr: 'not found', status: 1 };
        ...
        it('rejects invalid rotation value', () => {
            ...
            expect(() => hw.setRotation(45)).toThrow('Rotation must be 0, 90, 180, or 270');
        });
    });
...
//# sourceMappingURL=display-hardware.test.js.map