...
function makeEspnResponse(games = []) {
    return {
        events: games,
    };
}
...
describe('Sports Server', () => {
    ...
    it('should release refreshing lock after fetch timeout', async () => {
        const hangingFetch = vi.fn().mockReturnValue(new Promise(() => { }) // never resolves
        );
        const server = createSportsServer({
            leagues: [{ sport: 'football', league: 'nfl' }],
            dataBus,
...
//# sourceMappingURL=sports-server.test.js.map