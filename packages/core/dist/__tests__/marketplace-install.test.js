...
describe('downloadAndInstallPlugin', () => {
    ...
    function mockZipResponse(zipBuffer) {
    ...
    describe('successful install', () => {
        it('should download ZIP and install plugin', async () => {
            ...
            const result = await downloadAndInstallPlugin('https://example.com/plugins/test-plugin-1.0.0.zip', tmpDir, { fetchFn: mockFetch });
        ...
        it('should call fetch with correct URL', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith('https://cdn.example.com/my-plugin.zip', expect.objectContaining({
                method: 'GET',
    ...
    describe('size validation', () => {
        it('should reject downloads exceeding max size (default 10MB)', async () => {
            ...
            await expect(downloadAndInstallPlugin('https://example.com/large.zip', tmpDir, { fetchFn: mockFetch })).rejects.toThrow(/size|10.*MB|too large/i);
        });
        it('should allow custom max size', async () => {
            ...
            mockFetch.mockResolvedValueOnce({
                ...
                status: 200,
                statusText: 'OK',
                arrayBuffer: async () => mediumBuffer.buffer.slice(mediumBuffer.byteOffset, mediumBuffer.byteOffset + mediumBuffer.byteLength),
            });
            await expect(downloadAndInstallPlugin('https://example.com/medium.zip', tmpDir, {
                fetchFn: mockFetch,
    ...
    describe('download error handling', () => {
        it('should handle HTTP error responses', async () => {
            ...
            await expect(downloadAndInstallPlugin('https://example.com/missing.zip', tmpDir, {
                fetchFn: mockFetch,
            })).rejects.toThrow(/404|Not Found|download/i);
        });
        it('should handle network errors', async () => {
            mockFetch.mockRejectedValueOnce(new Error('Connection refused'));
            await expect(downloadAndInstallPlugin('https://example.com/plugin.zip', tmpDir, {
                fetchFn: mockFetch,
        ...
        it('should handle timeout', async () => {
            ...
            await expect(downloadAndInstallPlugin('https://example.com/slow.zip', tmpDir, {
                fetchFn: mockFetch,
                timeoutMs: 50,
    ...
    describe('SSRF protection', () => {
        it('should reject localhost download URLs', async () => {
            await expect(downloadAndInstallPlugin('http://localhost:8080/plugin.zip', tmpDir, {
                fetchFn: mockFetch,
            })).rejects.toThrow(/blocked|localhost/i);
            expect(mockFetch).not.toHaveBeenCalled();
        });
        it('should reject private IP download URLs', async () => {
            await expect(downloadAndInstallPlugin('http://192.168.1.1/plugin.zip', tmpDir, {
                fetchFn: mockFetch,
    ...
    describe('domain allowlist', () => {
        it('should reject download from domain not in allowlist', async () => {
            const zipBuffer = makeZip({ 'plugin.json': VALID_MANIFEST });
            mockZipResponse(zipBuffer);
            await expect(downloadAndInstallPlugin('https://evil.com/plugin.zip', tmpDir, {
                fetchFn: mockFetch,
        ...
        it('should allow download from domain in allowlist', async () => {
            ...
            expect(result.pluginId).toBe('test-plugin');
            expect(mockFetch).toHaveBeenCalledWith('https://plugins.example.com/test.zip', expect.any(Object));
        });
...
//# sourceMappingURL=marketplace-install.test.js.map