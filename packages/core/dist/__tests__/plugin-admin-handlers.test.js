...
describe('PluginAdminHandlers (plugin-admin-handlers.ts)', () => {
    ...
    it('should return plugin with defaults when no DB state exists', async () => {
        ...
        fs.writeFileSync(path.join(pluginsDir, 'test-plugin', 'plugin.json'), JSON.stringify({
            id: 'test-plugin',
            name: 'Test Plugin',
            version: '1.0.0',
        }));
    ...
    it('should persist zone assignment to DB', async () => {
        ...
        await loader.load();
        const handlers = createPluginAdminHandlers({ pluginLoader: loader, db });
        await handlers.assignPluginZone('test-plugin', 'top-bar');
        const plugin = await handlers.getPlugin('test-plugin');
    ...
    it('should reload plugins via plugin loader', async () => {
        ...
        fs.writeFileSync(path.join(pluginsDir, 'test-plugin-2', 'plugin.json'), JSON.stringify({
            id: 'test-plugin-2',
            name: 'Test Plugin 2',
            version: '1.0.0',
        }));
    ...
    it('should merge multiple mutations into single persisted state', async () => {
        ...
        await handlers.assignPluginZone('test-plugin', 'left-col');
        const plugin = await handlers.getPlugin('test-plugin');
        expect(plugin.enabled).toBe(false);
    ...
    it('should persist zone for built-in modules via assignPluginZone', async () => {
        ...
        await loader.load();
        const handlers = createPluginAdminHandlers({ pluginLoader: loader, db });
        await handlers.assignPluginZone('weather', 'right-col');
        const entry = await handlers.getPlugin('weather');
    ...
    it('should return integration_status: not_needed for modules with no integration fields', async () => {
        ...
        for (const moduleId of ['crypto', 'news', 'sports']) {
    ...
    describe('getPluginTemplate', () => {
        it('should return html and css for a builder-created plugin', async () => {
            ...
            fs.writeFileSync(path.join(pluginsDir, 'my-widget', 'plugin.json'), JSON.stringify({ id: 'my-widget', name: 'My Widget', version: '1.0.0' }));
        ...
        it('should return undefined for plugin without template files', async () => {
            ...
            fs.writeFileSync(path.join(pluginsDir, 'no-template', 'plugin.json'), JSON.stringify({ id: 'no-template', name: 'No Template', version: '1.0.0' }));
            await loader.load();
            const handlers = createPluginAdminHandlers({ pluginLoader: loader, db, pluginsDir });
...
//# sourceMappingURL=plugin-admin-handlers.test.js.map