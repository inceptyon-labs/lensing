...
function createMockDb() {
    ...
    return {
        ...
        getPluginState: (id) => state.get(`plugin:${id}`),
...
describe('Plugin Admin Handlers — Connector Lifecycle', () => {
    ...
    function createPluginWithConnector(id, connectorType = 'json_api') {
    ...
    describe('enable/disable with connector lifecycle', () => {
        ...
        it('calls unregister (safe no-op) when disabling a plugin without a connector', async () => {
            ...
            fs.mkdirSync(dir, { recursive: true });
            fs.writeFileSync(path.join(dir, 'plugin.json'), JSON.stringify({ id: 'static-widget', name: 'Static Widget', version: '1.0.0' }));
            const loader = createPluginLoader({
                pluginsDir: TEMP_PLUGINS_DIR,
                connectorRunner: mockConnectorRunner,
            });
        ...
        it('disabled connector stays unregistered after reloadPlugins', async () => {
            ...
            // Disable the plugin
            await handlers.setPluginEnabled('api-widget', false);
            mockConnectorRunner.unregister.mockClear();
            mockConnectorRunner.register.mockClear();
            // Reload all plugins
...
//# sourceMappingURL=plugin-admin-handlers-lifecycle.test.js.map