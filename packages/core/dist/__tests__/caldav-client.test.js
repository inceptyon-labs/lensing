...
describe('CalendarServer', () => {
    describe('factory and config', () => {
        ...
        it('should throw if serverUrl does not use HTTPS', () => {
            expect(() => createCalendarServer(validOptions({ serverUrl: 'http://caldav.icloud.com' }))).toThrow(/https/i);
        });
...
END:VEVENT</calendar-data>
    ...
    describe('dataBus publishing', () => {
        it('should publish events to dataBus on successful refresh', async () => {
            ...
            expect(publish).toHaveBeenCalledWith('calendar.events', 'calendar-server', expect.objectContaining({
    ...
    describe('timeout', () => {
        it('should timeout if CalDAV server does not respond within timeout window', async () => {
            const fetchFn = vi.fn(() => new Promise((_, reject) => {
...
//# sourceMappingURL=caldav-client.test.js.map