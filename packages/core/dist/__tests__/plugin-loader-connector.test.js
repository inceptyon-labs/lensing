...
function makeManifest(id, extra = {}) {
    return JSON.stringify({ id, name: `${id} Widget`, version: '1.0.0', ...extra });
}
function makeConnector(config) {
    return JSON.stringify(config);
}
function createPluginDir(id, manifest = makeManifest(id), connector) {
...
describe('Plugin Loader — Connector Integration', () => {
    ...
    describe('connector.json discovery', () => {
        ...
        it('loads without a connectorRunner even when connector.json exists', async () => {
            createPluginDir('static-no-runner', makeManifest('static-no-runner'), makeConnector({ type: 'static_data', url: '' }));
            const loader = createPluginLoader({ pluginsDir: TEMP_PLUGINS_DIR });
            const plugins = await loader.load();
        ...
        it('calls register for a plugin that has connector.json', async () => {
            createPluginDir('api-widget', makeManifest('api-widget'), makeConnector({
                type: 'json_api',
            ...
            expect(mockRunner.register).toHaveBeenCalledWith('api-widget', expect.objectContaining({ id: 'api-widget' }), expect.objectContaining({
                type: 'json_api',
                url: 'https://api.example.com/data',
                refreshInterval: 60,
            }));
        ...
        it('registers all plugins that have connectors', async () => {
            ...
            const calls = mockRunner.register.mock.calls;
            const registeredIds = calls.map((c) => c[0]);
    ...
    describe('connector.json parsing', () => {
        it('passes full connector config to register', async () => {
            createPluginDir('full-config', makeManifest('full-config'), makeConnector({
                type: 'json_api',
                url: 'https://api.example.com/data',
                method: 'POST',
            ...
            await loader.load();
            expect(mockRunner.register).toHaveBeenCalledWith('full-config', expect.any(Object), expect.objectContaining({
                type: 'json_api',
                url: 'https://api.example.com/data',
                method: 'POST',
                headers: { 'X-Api-Key': 'secret' },
        ...
        it('handles static_data connector type', async () => {
            createPluginDir('static-widget', makeManifest('static-widget'), makeConnector({ type: 'static_data', url: '', data: { value: 42, label: 'hello' } }));
            const loader = createPluginLoader({
                pluginsDir: TEMP_PLUGINS_DIR,
                connectorRunner: mockRunner,
            });
            await loader.load();
            expect(mockRunner.register).toHaveBeenCalledWith('static-widget', expect.any(Object), expect.objectContaining({ type: 'static_data' }));
        });
    ...
    describe('unload', () => {
        it('calls unregister when a plugin with a connector is unloaded', async () => {
            createPluginDir('to-unload', makeManifest('to-unload'), makeConnector({ type: 'json_api', url: 'https://api.example.com/data' }));
            const loader = createPluginLoader({
                pluginsDir: TEMP_PLUGINS_DIR,
                connectorRunner: mockRunner,
            });
    ...
    describe('reload', () => {
        it('unregisters old connectors and re-registers on reload', async () => {
            createPluginDir('reload-widget', makeManifest('reload-widget'), makeConnector({ type: 'json_api', url: 'https://api.example.com/v1' }));
            const loader = createPluginLoader({
                pluginsDir: TEMP_PLUGINS_DIR,
                connectorRunner: mockRunner,
            });
            ...
            const lastCall = mockRunner.register.mock.calls.at(-1);
            expect(lastCall[2]).toMatchObject({ url: 'https://api.example.com/v2' });
        });
    });
});
//# sourceMappingURL=plugin-loader-connector.test.js.map