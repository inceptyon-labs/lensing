...
function createMockDeps() {
    return {
        dataBus: { publish: vi.fn(), subscribe: vi.fn(), close: vi.fn() },
        notifications: { emit: vi.fn(), close: vi.fn() },
    };
}
describe('bootEnabledModules', () => {
    ...
    it('should call refresh() immediately after booting polling modules', async () => {
        ...
        if (pirInstance) {
            expect(pirInstance.refresh).toBeUndefined();
        }
    });
    ...
    it('should pass dataBus to weather module', () => {
        ...
        expect(createWeatherServer).toHaveBeenCalledWith(expect.objectContaining({ dataBus: deps.dataBus }));
    });
    it('should boot photo-slideshow module with photoDirectory', () => {
        ...
        expect(createPhotoSlideshowServer).toHaveBeenCalledWith(expect.objectContaining({ photoDir: '/home/pi/photos', dataBus: deps.dataBus }));
    });
    it('should pass dataBus to calendar module', () => {
        ...
        bootEnabledModules(db, deps);
        expect(createCalendarServer).toHaveBeenCalledWith(expect.objectContaining({ dataBus: deps.dataBus }));
    });
});
describe('rebootModule', () => {
    ...
    it('should reboot a running module (old closed, new in array)', () => {
        const oldClose = vi.fn();
        const modules = [{ id: 'weather', instance: { close: oldClose } }];
        db.setSetting('weather.enabled', 'true');
        db.setSetting('weather.apiKey', 'new-key');
        db.setSetting('weather.lat', '40.7');
        db.setSetting('weather.lon', '-74');
    ...
    it('should return null for unknown module ID', () => {
        const modules = [];
        const result = rebootModule('nonexistent', modules, db, deps);
...
describe('syncModulesWithLayout', () => {
    ...
    it('should not restart modules already running that are still in layout', () => {
        ...
        const modules = [{ id: 'crypto', instance: existingInstance }];
        const result = syncModulesWithLayout(['crypto'], modules, db, deps);
        expect(result).toHaveLength(1);
    ...
    it('should clear polling timers when stopping a module', () => {
        ...
        const modules = [{ id: 'crypto', instance: { close: closeFn }, timer }];
...
//# sourceMappingURL=module-boot.test.js.map