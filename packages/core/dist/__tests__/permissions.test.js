...
import { validateNetworkDomain, validateRefreshRate, validateSecretAccess, createPermissionEnforcer, } from '../plugin-permissions';
describe('Permission Validators', () => {
    describe('validateNetworkDomain', () => {
        ...
        it('should block requests to undeclared domains', () => {
            ...
            expect(validateNetworkDomain('https://api.example.com.evil.com/data', permissions)).toBe(false);
        });
...
describe('PermissionEnforcer', () => {
    ...
    describe('enforced fetch', () => {
        ...
        it('should block fetch to unauthorized domains', async () => {
            const enforcedFetch = enforcer.createFetchProxy(mockFetch);
            await expect(enforcedFetch('https://evil.com/data')).rejects.toThrow('Permission denied: domain not allowed');
        ...
        it('should block fetch with invalid URL', async () => {
            const enforcedFetch = enforcer.createFetchProxy(mockFetch);
            await expect(enforcedFetch('not a valid url')).rejects.toThrow('Permission denied: domain not allowed');
            expect(violations.length).toBe(1);
            expect(violations[0].type).toBe('network');
    ...
    describe('host-level enforcement', () => {
        ...
        it('should provide isolated context for each plugin', async () => {
            ...
            await expect(fetch1('https://domain2.com/data')).rejects.toThrow('Permission denied: domain not allowed');
            // Plugin 2 can access domain2, but NOT domain1
            await expect(fetch2('https://domain2.com/data')).resolves.toBeDefined();
            await expect(fetch2('https://domain1.com/data')).rejects.toThrow('Permission denied: domain not allowed');
