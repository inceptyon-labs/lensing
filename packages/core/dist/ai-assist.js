...
function validateOutput(raw) {
    ...
    if (!['json_api', 'rss_feed', 'static_data'].includes(connector.type)) {
    ...
    return {
        async generate(input) {
            ...
            if (docsText.length > maxDocsSize) {
                throw new Error(`Documentation text exceeds maximum size of ${maxDocsSize} characters (got ${docsText.length})`);
            }
            const contextLines = [];
...
Generate the widget configuration JSON now.`;
            ...
            try {
                ...
                throw new Error(`Failed to parse JSON from LLM response: ${err instanceof Error ? err.message : String(err)}`);
            }
