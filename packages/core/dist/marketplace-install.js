...
export async function downloadAndInstallPlugin(downloadUrl, pluginsDir, options) {
    const { fetchFn = fetch, timeoutMs = DEFAULT_TIMEOUT_MS, maxSizeBytes = DEFAULT_MAX_SIZE_BYTES, replace = false, allowedDomains, } = options ?? {};
    ...
    if (allowedDomains && allowedDomains.length > 0) {
        ...
        if (!allowed.includes(hostname)) {
            throw new Error(`Download domain "${hostname}" is not in the allowlist: ${allowedDomains.join(', ')}`);
        }
    }
...
//# sourceMappingURL=marketplace-install.js.map