 ...
 */
export function createDisplayHardware(options = {}) {
    ...
    return {
        ...
        setBrightness(value) {
            ...
            if (brightnessCap.method === 'ddcutil') {
                ...
                if (result.status !== 0)
                    throw new Error(`ddcutil setvcp failed: ${result.stderr}`);
            }
        },
        getContrast() {
            if (!contrastCap.available)
                return null;
        ...
        setContrast(value) {
            if (!contrastCap.available)
                throw new Error('Contrast control not available');
            if (value < 0 || value > 100)
                throw new Error('Contrast must be 0–100');
        ...
        getRotation() {
            ...
            if (result.status !== 0)
                throw new Error(`xrandr query failed: ${result.stderr}`);
            ...
            for (const line of lines) {
                ...
                if (match) {
                    ...
                    if (segment in ROTATION_REVERSE)
                        return ROTATION_REVERSE[segment];
                }
        ...
        setRotation(value, persistent) {
            ...
            if (result.status !== 0)
                throw new Error(`xrandr rotate failed: ${result.stderr}`);
            if (persistent) {
                try {
...
//# sourceMappingURL=display-hardware.js.map