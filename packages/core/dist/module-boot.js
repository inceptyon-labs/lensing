import { MODULE_SCHEMAS, MODULE_IDS, resolveCategoriesToFeeds, AI_NEWS_SCHEDULES, AI_NEWS_CATEGORIES, } from '@lensing/types';
 ...
 */
export function rebootModule(id, modules, db, deps, log) {
...
export function bootEnabledModules(db, deps, log) {
    const booted = [];
 ...
 */
export function syncModulesWithLayout(layoutIds, modules, db, deps, log) {
...
function getAiNewsRefreshMs(values) {
    ...
    switch (id) {
        case 'weather':
            return createWeatherServer({
                ...
                location: values['lat'] != null && values['lon'] != null
        ...
        case 'pir':
            return createPIRServer({
                ...
                idleTimeout_ms: values['idleTimeout_ms'] != null ? Number(values['idleTimeout_ms']) : undefined,
                gpioFactory,
                dataBus,
            });
        ...
        case 'ai-news': {
            ...
            if (aiNewsFeedUrls.length === 0)
                return null; // No categories selected
            ...
            return createAiNewsServer({
                ...
                summarize: async (articles) => {
                    ...
                    const response = await provider.generate([
                        {
                            role: 'user',
                            content: `Summarize each of the following news headlines in 2-3 concise sentences. ` +
...
//# sourceMappingURL=module-boot.js.map