...
// ── Factory ────────────────────────────────────────────────────────────────
export function createCryptoServer(options) {
    const { watchlist, alertConfigs = [], dataBus, notifications, fetchFn = fetch, maxStale_ms = DEFAULT_MAX_STALE_MS, } = options;
    ...
    function checkAndEmitAlerts(coins) {
        for (const config of alertConfigs) {
            ...
            if (Math.abs(change) >= config.threshold_pct) {
                ...
                dataBus.publish(DATA_BUS_ALERTS_CHANNEL, PLUGIN_ID, {
    ...
    async function refresh() {
        ...
        try {
            ...
            dataBus.publish(DATA_BUS_PRICES_CHANNEL, PLUGIN_ID, publishData);
...
//# sourceMappingURL=crypto-server.js.map