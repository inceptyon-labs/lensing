...
describe('startServer', () => {
    ...
    it('should call createHostService with CLI defaults', async () => {
        await startServer();
        expect(mockCreateHostService).toHaveBeenCalledWith(expect.objectContaining({
            port: 3100,
            pluginsDir: './plugins',
    ...
    it('should allow overriding pluginsDir', async () => {
        ...
        expect(mockCreateHostService).toHaveBeenCalledWith(expect.objectContaining({ pluginsDir: '/custom/plugins' }));
    });
    it('should allow overriding dbPath', async () => {
        await startServer({ dbPath: '/tmp/test.db' });
        expect(mockCreateHostService).toHaveBeenCalledWith(expect.objectContaining({ dbPath: '/tmp/test.db' }));
    });
    ...
    it('should log startup info via provided logger', async () => {
        ...
        const logMessages = logger.info.mock.calls.map((c) => c[0]);
        expect(logMessages.some((msg) => typeof msg === 'string' && (msg.includes('3100') || msg.includes('listening')))).toBe(true);
    });
