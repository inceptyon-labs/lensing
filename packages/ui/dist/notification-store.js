export function createNotificationStore(options = {}) {
    ...
    const pluginToggles = new Map();
    let quietHours;
    function notify(action, id) {
        onChange?.(action, id);
    }
    ...
    function isPluginEnabled(source) {
        return pluginToggles.get(source) ?? true;
    }
    function shouldSuppress(notification) {
    ...
    return {
        ...
        getActiveToasts() {
            return Array.from(notifications.values())
                .filter((e) => (e.priority === 'info' || e.priority === 'warning') &&
                !e.dismissed &&
                !isExpired(e) &&
